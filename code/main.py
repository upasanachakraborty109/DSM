...
for Run in range(0, num_Run):
    ...
    num_F = np.zeros([num_Step, num_Eps], dtype=object)
    num_R = np.zeros([num_Step, num_Eps], dtype=object)
    ...
    grid_states = np.zeros([Dim_L, Dim_W],dtype=object)

    X_Mat = np.zeros([num_Step, num_Eps, num_UAV], dtype=object)
    Y_Mat = np.zeros([num_Step, num_Eps, num_UAV], dtype=object)

    State_Mat = np.zeros([num_Step, num_Eps, num_UAV], dtype=object)
    next_state_index = np.zeros([num_Step, num_Eps, num_UAV], dtype=object)

    action = np.zeros([num_Step, num_Eps, num_UAV], dtype=object)
    task_matrix = np.zeros([num_Step, num_Eps, num_UAV], dtype=object)
    prev_task = np.zeros([num_Step, num_Eps, num_UAV], dtype=object)
    task_diff = np.zeros([num_Step, num_Eps], dtype=object)

    seed(a=None)
    #########################################################
    # Main Function of the Simulation
    qVal = np.zeros([num_states, num_states, num_action, num_action],dtype=object)
    timer = 0
    for Eps in range(0, num_Eps):
        timer = time.perf_counter()
        number_meet = np.zeros(num_UAV * [num_states], dtype=object)
    ...
    outputFile = 'dataOut_greedy_Size_%d_Run_%d_Eps_%d_Step_%d.npz' % (Size, Run, num_Eps, num_Step)  # Windows
