...
def get_csi(num_UAV, location, x_u, y_u):
    ...
    csi_h[:, source_uav] = np.squeeze(h_S_uav).shape
    csi_h[:, uav_fusion] = np.squeeze(h_uav_F).shape
    csi_h[:, gtuser_uav] = np.squeeze(h_GT_uav).shape
    csi_h[:, uav_gruser] = np.squeeze(h_uav_GR).shape
    return csi_h
